...
class QRCodeDetectorNode : public rclcpp::Node {
      ...
      void image_callback(const sensor_msgs::msg::Image::ConstSharedPtr& msg) {
          try {
              cv::Mat frame = cv_bridge::toCvShare(msg, "bgr8")->image;
               ...
               std::vector<cv::Point> avg_qr_pixels(4, cv::Point(0, 0));
               cv::Mat avg_cube_pixels = cv::Mat::zeros(8,2, CV_32F);
               float avg_center_x = 0;
               float avg_center_y = 0;
               float avg_center_z = 0;


               cv::Mat avg_rvec(3, 1, CV_32F, cv::Scalar(0));


               //get values from multiple frames
               for (int i = 0; i < 10; i++){
                   //RCLCPP_WARN(this->get_logger(), "frame loop");
                   detect_cube_cords(frame, successful_frames, avg_qr_pixels, avg_cube_pixels, avg_center_x, avg_center_y, avg_center_z, avg_rvec);
               }
               if (successful_frames){
                   //RCLCPP_WARN(this->get_logger(), "doing averaging");


                   //average values based on successful values
                   for (int i = 0; i < 4; i++){
                       avg_qr_pixels[i].x /= successful_frames;
                       avg_qr_pixels[i].y /= successful_frames;
                   }
                   for (int i = 0; i < 8; i++){
                       avg_cube_pixels.at<float>(i, 0) /= successful_frames;
                       avg_cube_pixels.at<float>(i, 1) /= successful_frames;
                   }


                   for (int i = 0; i < 3; i++){
                       avg_rvec.at<float>(i, 0) /= successful_frames;
                       avg_rvec.at<float>(i, 1) /= successful_frames;
                       avg_rvec.at<float>(i, 2) /= successful_frames;
                   }
                   avg_center_x /= successful_frames;
                   avg_center_y /= successful_frames;
                   avg_center_z /= successful_frames;


                   //draw and publish results
                   drawPoints(frame, avg_qr_pixels, 0, 255, 0);
                   drawBoundingBox(frame, avg_cube_pixels, 255, 0, 0);


                   //publish pose data
                   geometry_msgs::msg::Pose pose_msg;
                   pose_msg.position.x = avg_center_x;
                   pose_msg.position.y = avg_center_y;
                   pose_msg.position.z = avg_center_z;
                   pose_msg.orientation.x = avg_rvec.at<float>(0,0);
                   pose_msg.orientation.y = avg_rvec.at<float>(1,0);
                   pose_msg.orientation.z = avg_rvec.at<float>(2,0);
  ...
  void detect_cube_cords(cv::Mat& image, int &successful_frames, std::vector<cv::Point> &avg_qr_pixels, cv::Mat &avg_cube_pixels, float &avg_center_x, float &avg_center_y, float &avg_center_z, cv::Mat &avg_rvec) {
      ...
      //get pixel coords of cube corners
       ...
       cv::Mat cube_rvec(3, 1, CV_32F, cv::Scalar(0));
      ...
      if (qrDecoder.detect(image, qrCode_pixel_cords)){
           //RCLCPP_WARN(this->get_logger(), "detect success");
           //RCLCPP_WARN(this->get_logger(), "qr code pixels length: %li", qrCode_pixel_cords.size());
           bool error_passed = false;
           errorCheckBoundingBox(qrCode_pixel_cords, error_passed);


           if (error_passed){//find displacement of 8 cube corners


               //RCLCPP_WARN(this->get_logger(), "error check success");
              ...
              if (cv::solvePnP(qr_corners, pixelsMat, INTRINSIC, DIST, rvec, tvec)){
               //RCLCPP_WARN(this->get_logger(), "solve pnp 1 success");
              ...
              //*************************************************************************************************************************************
                  ...
                  if (cv::solvePnP(cube_corners_coplanar_front, frontPixels, INTRINSIC, DIST, cube_rvec, cube_tvec)){ //solve for rotation and translation
                       //RCLCPP_WARN(this->get_logger(), "solve pnp 2 success");
                      ...
                      for (int i = 0; i<3; i++) center[i] = (center[i] / cube_xyz.rows) * scale_;
                       ...
                       //RCLCPP_WARN(this->get_logger(), "center calc success");
                       ...
                       //add qr pixels
                       for (int i = 0; i < 4; i++){
                           avg_qr_pixels[i].x += qrCode_pixel_cords[i].x;
                           avg_qr_pixels[i].y += qrCode_pixel_cords[i].y;
                       }
                       //RCLCPP_WARN(this->get_logger(), "avg qr pixels success");


                       //cube pixels
                       for (int i = 0; i < 8; i++){
                           avg_cube_pixels.at<float>(i, 0) += cube_pixels.at<float>(i, 0);
                           avg_cube_pixels.at<float>(i, 1) += cube_pixels.at<float>(i, 1);
                       }
                       //RCLCPP_WARN(this->get_logger(), "avg cube pixels success");




                       for (int i = 0; i < 3; i++){
                           avg_rvec.at<float>(i, 0) += cube_rvec.at<float>(i, 0);
                       }
                       //RCLCPP_WARN(this->get_logger(), "avg rvec success");




                       avg_center_x += center[0];
                       avg_center_y += center[1];
                       avg_center_z += center[2];
                       //RCLCPP_WARN(this->get_logger(), "avg center success");


                       //std::cout<<"finished_detect"<<std::endl;
           ...
           //if error check passes we add values to averages at the end of calculations
           //and increment successful_frames counter for divison outside of detect_cube_cords
  ...
  void errorCheckBoundingBox(const std::vector<cv::Point> pixels, bool &success) {
       //RCLCPP_WARN(this->get_logger(), "enter error check");
      ...
      int bottom_left_y = pixels[3].y;
       ...
       //RCLCPP_WARN(this->get_logger(), "variable assignment");
      ...
      if (top_dist_squared < 0.75 * right_dist_squared || top_dist_squared > 1.3 * right_dist_squared) {
           //RCLCPP_WARN(this->get_logger(), "dist failed");
          return; // Top dist is less than 90% or greater than 110% of right distance
      }


       //RCLCPP_WARN(this->get_logger(), "dist passed");
      ...
      if (angle_left_right_pair > 10 || angle_top_bottom_pair > 10) {
           //RCLCPP_WARN(this->get_logger(), "angle failed");
          return;
      }


       //RCLCPP_WARN(this->get_logger(), "angle passed");
